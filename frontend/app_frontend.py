...
# تعديل المعامل إلى unsafe_allow_html=True لمنع الكراش في السيرفر
st.markdown("""
    <style>
...
# ==========================================
# 2. شاشة محادثة الذكاء الاصطناعي (RAG Query)
# ==========================================
elif menu == "💬 اسأل الذكاء الاصطناعي عن المدارس (RAG)":
    st.header("💬 اسأل الذكاء الاصطناعي عن المدارس (RAG)")
    st.write("اختر مدرسة واسأل المساعد الذكي عن شروطها وتفاصيلها من واقع ملفاتها الرسمية المعتمدة.")
    ...
    with st.spinner("جاري تهيئة قاعدة المعرفة الذكية..."):
        try:
            ...
            if schools_res.status_code == 200 and schools_res.json():
                ...
                user_question = st.text_input("أدخل سؤالك هنا:")
                        ...
                        with st.spinner("جاري قراءة مستندات المدرسة بدقة وتوليد الإجابة الحتمية..."):
                            try:
                                ...
                                if response.status_code == 200:
                                    ...
                                    with st.expander("🔍 المصادر والمستندات المستند إليها:"):
...
# ==========================================
    ...
    st.header("➕ إضافة مدرسة جديدة يدوياً")
    ...
    with st.form("add_school_form"):
        ...
        accepted_governorates = st.text_input("المحافظات المقبولة:", value="All")
...
# ==========================================
    ...
    st.header("🔍 اضافة مدرسة باستخدام نص عشوائي")
    
    school_text = st.text_area("أدخل النص التعريفي بالمدرسة:", height=150)
            ...
            with st.spinner("جاري التحليل واستخراج البيانات وحفظها..."):
                try:
                    ...
                    if response.status_code == 200:
                        ...
                        st.success("🎉 تم استخراج البيانات وحفظ المدرسة بنجاح!")
                        st.json(school_data)
                    else:
    ...
    with st.spinner("جاري جلب قائمة المدارس النشطة..."):
        try:
            ...
            if schools_res.status_code == 200 and schools_res.json():
                ...
                selected_school_name = st.selectbox("اختر المدرسة التي تود رفع هذا الدليل والملف لها:", list(school_options.keys()))
                selected_school_id = school_options[selected_school_name]
                ...
                if st.button("ابدأ المعالجة وتكشيف المتجهات"):
                    if uploaded_file is not None:
                        with st.spinner("جاري حفظ المتجهات في نطاق المدرسة المحددة..."):
                            try:
                                files = {"file": (uploaded_file.name, uploaded_file.getvalue(), "application/pdf")}
                                res = requests.post(f"{API_BASE_URL}/rag/ingest?school_id={selected_school_id}", files=files, headers=headers)
                                
                                if res.status_code == 200:
                                    st.success(f"🎉 تم تكشيف ورفع دليل مدرسة ({selected_school_name}) بنجاح وعزلها!")
                                    st.json(res.json())
                                else:
